...
if not re.search('mpirun|mpiexec', parent_name):
    ...
    # descriptor:
    try:
        subprocess.call(['/usr/local/envs/NK/bin/mpiexec', '--allow-run-as-root', '--oversubscribe', '-np', '1']+MPIEXEC_EXTRA_OPTS+\
        ...
        try:
            subprocess.call(['/usr/local/envs/NK/bin/mpiexec', '--allow-run-as-root', '--oversubscribe', '-np', '1',
                            sys.executable, script_name]+sys.argv[1:],
                            env=env)
